...
function resolveLive(config) {
  ...
  return {
    ...
    // v0.7.1: persistent dispatch state (spawned/callback keys, comment counts,
    // processed bridge callbacks, lastPollAt) survives restarts — the in-memory
    // closure state was the root cause of the restart storms (see bootDrain).
    stateFile:
      typeof cfg.state?.file === 'string' && cfg.state.file.length > 0
        ? (cfg.state.file.startsWith('~/') ? join(homedir(), cfg.state.file.slice(2)) : cfg.state.file)
        : join(homedir(), '.dsh', 'taskboard-flow-state.json'),
    // v0.7.1 boot drain policy for execute-mode (todo) columns:
    //   'fresh' (default) — drain ONLY tasks enqueued while the plugin was
    //       down (updatedAt > persisted lastPollAt) with a clean execution
    //       history; everything else in todo is seeded as already-fired.
    //   'off'   — never dispatch at boot; seed everything (a queued card then
    //       waits for an explicit UI Run / orchestrator re-queue).
    //   'all'   — legacy v0.4.2 behavior (drain every todo card at boot).
    //       Known hazard: stale/rejected/failed cards re-spawn on EVERY
    //       restart — kept only for compatibility.
    bootDrain: cfg.bootDrain === 'off' || cfg.bootDrain === 'all' ? cfg.bootDrain : 'fresh',
  }
}
...
// ── Persistent dispatch state (v0.7.1) ─────────────────────────────────
// Restarts used to lose every dedupe key (spawnedTasks, callbackFired,
// processedCallbacks, comment counts), which re-armed all dispatch rules at
// boot: every stale todo card re-executed ("drain at boot") and every pending
// Telegram callback re-created its task. The state file restores the keys so
// a restart is observationally silent — only transitions seen AFTER the
// process came up fire.

const STATE_PROCESSED_CALLBACKS_CAP = 500
...
function loadState(file) {
  try {
    ...
    if (data && typeof data === 'object' && data.version === 1) {
      return {
        lastPollAt: Number.isFinite(data.lastPollAt) ? data.lastPollAt : 0,
        spawnedTasks: Array.isArray(data.spawnedTasks) ? data.spawnedTasks.filter((k) => typeof k === 'string') : [],
        callbackFired: Array.isArray(data.callbackFired) ? data.callbackFired.filter((k) => typeof k === 'string') : [],
        processedCallbacks: Array.isArray(data.processedCallbacks)
          ? data.processedCallbacks.filter((k) => typeof k === 'string').slice(-STATE_PROCESSED_CALLBACKS_CAP)
          : [],
        seenCommentCounts:
          data.seenCommentCounts && typeof data.seenCommentCounts === 'object'
            ? Object.fromEntries(
                Object.entries(data.seenCommentCounts).filter(([id, n]) => typeof id === 'string' && Number.isFinite(n)),
              )
            : {},
      }
    }
  } catch {
    /* missing/corrupt state file → empty state (first run after upgrade) */
  }
  return { lastPollAt: 0, spawnedTasks: [], callbackFired: [], processedCallbacks: [], seenCommentCounts: {} }
}

function saveState(file, snapshot) {
  const payload =
    JSON.stringify(
      { version: 1, updatedAt: new Date().toISOString(), ...snapshot, processedCallbacks: snapshot.processedCallbacks.slice(-STATE_PROCESSED_CALLBACKS_CAP) },
      null,
      2,
    ) + '\n'
  mkdirSync(dirname(file), { recursive: true })
...
// Latest execution outcome guard (v0.7.1): a card whose most recent run ended
// 'failed' or 'cancelled' (quick-reject rolls a card back to todo) must never
// auto-execute again — the old rule re-fired the card the moment it re-entered
// todo, overriding the human's reject on the next poll. Re-queueing such a
// card is an explicit act (UI Run button / orchestrator run tool, both of
// which bypass this plugin's dispatch).
function lastExecutionOutcome(task) {
  const ex = Array.isArray(task?.executions) ? task.executions : []
  return ex.length > 0 ? ex[ex.length - 1]?.outcome : undefined
}
...
export function apply(ctx, config) {
  ...
  // v0.7.1: the four dispatch-dedupe structures are seeded from the persisted
  // state file so a restart does not re-arm every rule (boot-storm fix).
  const persisted = loadState(live.stateFile)
  ...
  // fire again).
  const spawnedTasks = new Set(persisted.spawnedTasks)
  ...
  const callbackFired = new Set(persisted.callbackFired) // `${taskId}:${status}` keys — one dshCallback fire per status entry
  ...
  const seenCommentCounts = new Map(Object.entries(persisted.seenCommentCounts)) // taskId → last-seen comment count (@executor relay diffing)
  const activeSessions = new Set()
  const processedCallbacks = new Set(persisted.processedCallbacks)
  const callbackTimers = new Map()
  let mcpSession = null

  // Debounced write-behind for the state file: poll passes mark it dirty, a
  // 1s timer coalesces the writes; dispose() flushes synchronously.
  let stateDirty = false
  let stateFlushTimer = null
  function persistStateNow() {
    // Prune keys of tasks that no longer exist or are terminally settled —
    // the file must not grow without bound across months of boards.
    const liveIds = new Set()
    const terminal = new Set(['done', 'canceled', 'cancelled'])
    try {
      for (const t of readLedgerTasks(live.ledgerPath)) {
        if (!t?.id) continue
        liveIds.add(t.id)
        if (terminal.has(t.status)) continue
        liveIds.add(`keep:${t.id}`)
      }
    } catch {
      /* ledger unreadable → skip pruning this write */
    }
    const keepKey = (k) => {
      if (liveIds.size === 0) return true
      const id = k.slice(0, k.lastIndexOf(':'))
      return liveIds.has(`keep:${id}`) // key of a live, non-terminal task
    }
    try {
      saveState(live.stateFile, {
        lastPollAt: Date.now(),
        spawnedTasks: [...spawnedTasks].filter(keepKey),
        callbackFired: [...callbackFired].filter(keepKey),
        processedCallbacks: [...processedCallbacks],
        seenCommentCounts: [...seenCommentCounts.entries()].filter(([id]) => liveIds.size === 0 || liveIds.has(id)),
      })
      stateDirty = false
    } catch (e) {
      log(`[taskboard-flow] state persist failed: ${e?.message}`)
    }
  }
  ...
  async function processPendingCallbacks(projectCfg, workspaceId) {
    ...
    try {
      ...
      for (const cb of callbacks) {
        ...
        markStateDirty() // v0.7.1: bridge dedupe survives restarts (capped at 500 in the state file)
        try {
  ...
  async function deliverSessionCallback(agentCtx, task, cb, prevStatus, projectCfg) {
    ...
    if (!agent && cb.resumeIfDead === true) {
      try {
        const handle = await agentCtx.agents.resume({ resumeSessionId: targetId })
...
${buildCallbackText(task, cb, prevStatus)}`,
  ...
  async function pollLedger(agentCtx, wsCtx) {
    ...
    // First poll: populate the observation baseline, don't spawn.
    // v0.7.1: dedupe keys restored from the persisted state file are ALREADY
    // in spawnedTasks/callbackFired — this pass only fills the gaps (first
    // run after upgrade, or keys pruned because the task was terminal when
    // the plugin last wrote state but has since been reopened).
    if (isFirstPoll && live.skipFirstPoll) {
      ...
      for (const task of tasks) {
        if (task?.id && task?.status) {
          ...
          if (!seenCommentCounts.has(task.id)) {
          ...
          if (!colCfg.enabled) continue
          const spawnKey = `${task.id}:${task.status}`
          if (spawnedTasks.has(spawnKey)) continue
          if (colCfg.spawnMode === 'execute') {
            // v0.7.1 boot drain policy — the old unconditional drain re-spawned
            // every stale/rejected/failed todo card on EVERY restart (the
            // restart-storm root cause). 'fresh' (default) drains only cards
            // genuinely enqueued while the plugin was down and with a clean
            // execution history; 'off' drains nothing; 'all' keeps v0.4.2.
            const outcome = lastExecutionOutcome(task)
            const dirtyHistory = outcome === 'failed' || outcome === 'cancelled'
            const queuedWhileDown = persisted.lastPollAt > 0 && (task.updatedAt ?? 0) > persisted.lastPollAt
            const drain =
              live.bootDrain === 'all' ||
              (live.bootDrain === 'fresh' && queuedWhileDown && !dirtyHistory)
            if (drain) {
              // leave the spawn key ABSENT so the dispatch pass below fires it
              drainedFresh++
            } else {
      ...
      isFirstPoll = false
      log(
        `[taskboard-flow] first poll: ${seen.size} tasks observed, ${seeded} keys seeded, ${drainedFresh} fresh todo cards queued for drain (bootDrain=${live.bootDrain}, state=${persisted.lastPollAt > 0 ? 'restored' : 'empty'})`,
      )
      markStateDirty()
      return
    }
    ...
    for (const task of tasks) {
      ...
      if (
        ...
        if (skipPfxs.length > 0 && skipPfxs.some((p) => (task.title ?? '').startsWith(p))) {
          ...
          // v0.7.1 failed/rejected guard: the most recent run ended badly
          // (quick-reject rolls a card back to todo with outcome 'cancelled';
          // crash/timeout paths end 'failed'). Auto re-executing such a card
          // the moment it re-enters todo overrode the human's reject on the
          // very next poll (reject loop). Re-queueing is explicit: UI Run
          // button / orchestrator run tool, both outside this dispatch.
          // Cards with a clean ('succeeded' or no-run) history re-plan and
          // re-execute as before.
          {
            const outcome = lastExecutionOutcome(task)
            if (outcome === 'failed' || outcome === 'cancelled') {
              if (!throttledTasks.has(`guard:${task.id}`)) {
                throttledTasks.add(`guard:${task.id}`)
                log(
                  `[taskboard-flow] execute suppressed ${task.id} "${task.title ?? ''}": last execution outcome=${outcome} — explicit re-queue required (UI Run / orchestrator)`,
                )
              }
              spawnedTasks.add(spawnKey)
              seen.set(task.id, task.status)
    ...
    // v0.7.1: persist the mutated dedupe state (debounced) + advance
    // lastPollAt so the next boot's 'fresh' drain only sees cards enqueued
    // after THIS poll.
    markStateDirty()
  }
    ...
    wsCtx.inject(['agents'], (agentCtx) => {
      ...
      async function deliverSessionMessage({ sid, target, message, wake = true, resumeIfDead = false }) {
        ...
        if (!agent && resumeIfDead === true) {
          try {
            const handle = await agentCtx.agents.resume({ resumeSessionId: target })
            agent = handle?.agent ?? handle
            resumed = true
          } catch (e) {
            ...
            disposeContactsTool = toolsSvcCc.register({
              ...
              async execute(args, exec) {
                try {
                  ...
                  if (action === 'add') {
                    // v0.7.3: sessionId optional — omitted (or "self")
                    // registers the CALLING session. Agents self-register
                    // by name only; researching their own session id was
                    // slow. An explicit id still lets a caller register a
                    // contact pointing at another session.
                    ...
                    log(`[taskboard-flow] contacts add "${name}" → ${sessionId}${selfReg ? ' (self)' : ''}`)
      ...
      return () => {
        ...
        // v0.7.1: flush pending dispatch state so a clean shutdown keeps the
        // dedupe keys (a crash loses at most the last 1s debounce window —
        // the 'fresh' drain then re-fires only genuinely recent todo cards).
        if (stateFlushTimer) {
