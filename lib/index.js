        ...
        try {
          ...
          if (toolsSvcMsg && typeof toolsSvcMsg.register === 'function') {
            disposeSessionMsgTool = toolsSvcMsg.register({
              ...
              description:
                'Send a message to another DSH session agent, or list live sessions. Delivery: on an IDLE target the FULL message text is rendered into the target conversation (steer, followup fallback) AND pushed into its runtime context (~30-min TTL); main GUI sessions start a turn only on user input, so the text is visible the moment anyone opens the target session and the agent reads it at its next turn. A BUSY target receives the runtime-context note only. Actions: "list" → live sessions [{id,status,contact?}]; "send" (target + message required) → deliver. TARGET: pass the REGISTERED CONTACT NAME (preferred — resolved live at call time, so a re-raised agent under the same name is always reached; use contacts action "list" to discover names); a raw "session-…" id still works but goes stale when agents are re-raised. Optional: wake (default true), resumeIfDead (default false — resume a dead target session first). Self-send refused.',
              parameters: {
                type: 'object',
                ...
                properties: {
                  ...
                  target: { type: 'string', description: 'Registered contact name (PREFERRED — resolved to its current session id at call time, immune to stale ids) or a raw "session-…" id.' },
              ...
              async execute(args, exec) {
                try {
                  ...
                  // name is PREFERRED and resolved live at call time, so a
                  // re-raised agent under the same name is always reached.
                  // A raw session id is still accepted verbatim (legacy),
                  // but goes stale when agents are re-raised.
                  let target = typeof args?.target === 'string' ? args.target.trim() : ''
                  let resolvedFrom = null
                  if (target && !/^session-/.test(target)) {
